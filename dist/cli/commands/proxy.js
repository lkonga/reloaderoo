...
Examples:
  ...
  $ reloaderoo proxy --log-level debug -- python mcp_server.py --port 8080
    `)
        .option('-w, --working-dir <directory>', 'Working directory for the child process', process.cwd())
        .option('-l, --log-level <level>', 'Log level (debug, info, notice, warning, error, critical)', 'info')
        .option('-f, --log-file <path>', 'Custom log file path (logs to stderr by default)')
        .option('-t, --restart-timeout <ms>', 'Timeout for restart operations in milliseconds', '30000')
        .option('-m, --max-restarts <number>', 'Maximum number of restart attempts (0-10)', '3')
        .option('-d, --restart-delay <ms>', 'Delay between restart attempts in milliseconds', '1000')
        .option('-q, --quiet', 'Suppress non-essential output')
        .option('--no-auto-restart', 'Disable automatic restart on crashes')
        .option('--debug', 'Enable debug mode with verbose logging')
        .option('--dry-run', 'Validate configuration without starting proxy')
        ...
        try {
            ...
            // Validate log level
            const validLogLevels = ['debug', 'info', 'notice', 'warning', 'error', 'critical'];
            ...
            const { HTTPS_PROXY, https_proxy, HTTP_PROXY, http_proxy, ALL_PROXY, all_proxy, NO_PROXY, no_proxy, ...childEnv } = process.env;
            const proxyConfig = {
